...
@section('content')
    ...
    @php
        $activeFilterKeys = ['status', 'priority_id', 'category_id', 'label_id', 'assigned_to', 'created_from', 'created_to', 'due_from', 'due_to', 'overdue'];
        $activeFilterCount = 0;
        ...
        function sortSupervisorUrl($column) {
            $params = request()->all();
        ...
        <div class="container-fluid">
            <div class="d-flex flex-wrap justify-content-between align-items-center gap-2">
                <div>
                    ...
                    <p class="text-muted small mb-0">Pantau dan kelola seluruh tiket support tim Anda</p>
                </div>
                ...
                <div class="card-header bg-white border-bottom-0 p-3 rounded-top-4">
                    <div class="d-flex flex-wrap align-items-center justify-content-between gap-3">
                        <div class="d-flex align-items-center gap-2">
                            <h4 class="fw-bold mb-0 text-dark me-2 fs-5">Daftar Tiket</h4>
                        </div>

                        {{-- Search & Filter Controls --}}
                        <form method="GET" action="{{ route('supervisor.tickets.index') }}" id="supervisorFilterForm" class="d-flex align-items-center gap-2 mb-0 ms-auto">
                            ...
                            <div class="input-group input-group-sm rounded-pill overflow-hidden border bg-light" style="width: 260px;">
                                ...
                                <input type="text" name="search" class="form-control bg-transparent border-0 shadow-none ps-1"
                                    placeholder="Search..." value="{{ request('search') }}" onchange="document.getElementById('supervisorFilterForm').submit()">
                            </div>
                            ...
                            <div class="dropdown">
                                <button type="button" class="btn btn-outline-secondary btn-sm rounded-3 position-relative d-inline-flex align-items-center justify-content-center p-2"
                                        id="filamentFilterBtnSupervisor" data-bs-toggle="dropdown" data-bs-auto-close="outside" aria-expanded="false" style="width: 36px; height: 36px;">
                                ...
                                <div class="dropdown-menu dropdown-menu-end p-3 shadow-lg border-0 rounded-4 mt-2" style="width: 360px; max-width: 90vw; z-index: 1050;" aria-labelledby="filamentFilterBtnSupervisor">
                                    <div class="d-flex align-items-center justify-content-between mb-3 border-bottom pb-2">
                                        <h6 class="fw-bold mb-0 text-dark">Filters</h6>
                                        <a href="{{ route('supervisor.tickets.index') }}" class="text-danger text-decoration-none small fw-semibold">Reset</a>
                                    </div>
                                    ...
                                    <div class="row g-3">
                                        ...
                                        <div class="col-12">
                                            <label class="form-label form-label-sm fw-medium text-secondary mb-1">Assigned Agent</label>
                                            <select name="assigned_to" class="form-select form-select-sm rounded-3">
                                                <option value="">All</option>
                                                @foreach ($agents as $ag)
                                                    <option value="{{ $ag->id }}" {{ request('assigned_to') == $ag->id ? 'selected' : '' }}>
                                                        {{ $ag->name }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        ...
                                        <div class="col-12">
                                            <div class="form-check form-switch pt-1">
                                                <input class="form-check-input" type="checkbox" name="overdue" id="overdueCheckSupervisor" value="1" {{ request('overdue') ? 'checked' : '' }}>
                                                <label class="form-check-label small fw-semibold text-danger" for="overdueCheckSupervisor">
                                                    Hanya Tiket Overdue
                                                </label>
                                            </div>
                                        </div>
                        ...
                        <table class="table table-hover align-middle mb-0">
                            <thead class="bg-light">
                                <tr class="text-secondary small fw-semibold">
                                    ...
                                    <th>Kategori</th>
                                    <th>
                                        <a href="{{ sortSupervisorUrl('priority_id') }}" class="text-secondary text-decoration-none d-inline-flex align-items-center gap-1">
                                            Prioritas
                                    ...
                                    <th>
                                        <a href="{{ sortSupervisorUrl('status') }}" class="text-secondary text-decoration-none d-inline-flex align-items-center gap-1">
                                            Status
                                    ...
                                    <th class="text-nowrap">
                                        <a href="{{ sortSupervisorUrl('due_at') }}" class="text-secondary text-decoration-none d-inline-flex align-items-center gap-1">
                                            Tenggat
                                    ...
                                    <th class="text-nowrap">
                                        <a href="{{ sortSupervisorUrl('created_at') }}" class="text-secondary text-decoration-none d-inline-flex align-items-center gap-1">
                                            Dibuat Pada
