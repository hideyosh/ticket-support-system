    ...
    <div class="app-content-header mb-3">
        <div class="container-fluid">
            <div class="d-flex flex-wrap justify-content-between align-items-center gap-2">
                <div>
                    <h3 class="mb-1 fw-bold">Edit Tiket</h3>
                    <p class="text-muted small mb-0">Ubah informasi dan detail tiket support.</p>
                </div>
                ...
                <div class="card-body">
                    <form action="{{ route('admin.tickets.update', $ticket->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="description" rows="6" class="form-control @error('description') is-invalid @enderror">{{ old('description', $ticket->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3 mt-3">
                            ...
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu w-100 p-2 shadow border-0 rounded-4"
                                    ...
                                    @php
                                        $selectedLabels = old('labels', $ticket->labels ? $ticket->labels->pluck('id')->toArray() : []);
                                    @endphp
                        ...
                        {{-- SECTION LAMPIRAN EKSISTING & BARU --}}
                        <div class="mb-4">
                            ...
                            @if($ticket->attachments && $ticket->attachments->count() > 0)
                                <div class="mb-3">
                                    <span class="small text-muted d-block mb-2">Lampiran saat ini:</span>
                                    <div class="d-flex flex-wrap gap-2">
                                        @foreach($ticket->attachments as $attachment)
                                            <div class="badge bg-light text-dark border d-inline-flex align-items-center gap-2 p-2 rounded-3">
                                                <i class="bi bi-file-earmark-text text-primary"></i>
                                                <a href="{{ asset('storage/' . $attachment->file_path) }}" target="_blank" class="text-decoration-none text-dark">
                            ...
                            <div class="form-text text-muted small">
                                Tambah lampiran baru (opsional). Format: PNG, JPG, JPEG, PDF, DOC, DOCX, XLS, XLSX (Maks. 2MB per file).
                            </div>
                            @error('attachments.*')
                                <div class="text-danger small mt-1">{{ $message }}</div>
                            @enderror
